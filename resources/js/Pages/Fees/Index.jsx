...
export default function Index({ classes, grandCollected, grandPending }) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="bg-white rounded-xl shadow p-5 border-l-4 border-green-500">
                        ...
                        <p className="text-3xl font-bold text-green-600">
                            Rs. {grandCollected?.toLocaleString()}
                        </p>
                    </div>
                    <div className="bg-white rounded-xl shadow p-5 border-l-4 border-red-500">
                        <p className="text-sm text-gray-500">Total Pending</p>
                        <p className="text-3xl font-bold text-red-500">
                            Rs. {grandPending?.toLocaleString()}
                        </p>
                    </div>
                </div>
                                    ...
                                    >
                                        ...
                                        <td className="p-4">
                                            <span className="text-green-600 font-bold">
                                                Rs.{" "}
                                                {cls.total_collected?.toLocaleString()}
                                            </span>
                                        </td>
                                        <td className="p-4">
                                            <span
                                                className={`font-bold ${cls.total_pending > 0 ? "text-red-500" : "text-green-600"}`}
                                            >
                                                Rs.{" "}
                                                {cls.total_pending?.toLocaleString()}
                                            </span>
                                        </td>
                                        <td className="p-4">
                                            <Link
                                                href={`/fees/class-view?class_id=${cls.id}`}
